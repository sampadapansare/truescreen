...
user_roles = {}       # sid → {"room": ..., "role": "candidate"/"interviewer"}
...
def on_join(data):
    room = data['room']
    role = data.get('role', 'candidate')  # 'candidate' or 'interviewer'
    join_room(room)
    user_roles[request.sid] = {"room": room, "role": role}
    emit('user-joined', {'sid': request.sid, 'role': role}, room=room, include_self=False)
...
def on_disconnect():
    if request.sid in user_roles:
        del user_roles[request.sid]
